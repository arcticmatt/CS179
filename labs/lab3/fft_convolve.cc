...
/*************** END GAUSSIAN ******************/
    ...
    /* DONE: Allocate memory for these three arrays on the GPU.
    ...
    gpuErrchk(cudaMalloc((void **) &dev_input_data,
                padded_length * sizeof(cufftComplex)));
...
#define STOP_RECORD_TIMER(name) {                           \
    ...
    for (int ch = 0; ch < nChannels; ch++){
        ...
        /* DONE: Copy this channel's input data (stored in input_data)
        ...
        /* DONE: Copy this channel's impulse response data (stored in impulse_data)
        from host memory to the GPU.
        ...
        */
        ...
        /* DONE: We're only copying to part of the allocated
        ...
        */
        gpuErrchk(cudaMemset(dev_input_data + N, 0,
                    (padded_length - N) * sizeof(cufftComplex)));
        gpuErrchk(cudaMemset(dev_impulse_v + impulse_length, 0,
                    (padded_length - impulse_length) * sizeof(cufftComplex)));
        gpuErrchk(cudaMemset(dev_out_data, 0,
                    padded_length * sizeof(cufftComplex)));
        ...
        */
        ...
        /* DONE: Run the forward DFT on the input signal and the impulse response.
        (Do these in-place.) */
        ...
        /* DONE: Run the inverse DFT on the output signal.
        (Do this in-place.) */
        cufftExecC2C(plan, dev_out_data, dev_out_data, CUFFT_INVERSE);

        /* DONE: Destroy the cuFFT plan. */
        cufftDestroy(plan);
        ...
        /* DONE: Allocate memory to store the maximum magnitude found.
        (You only need enough space for one floating-point number.) */
        gpuErrchk(cudaMalloc((void **) &dev_max_abs_val,
                    sizeof(float)));

        /* DONE: Set it to 0 in preparation for running.
        ...
        /* DONE: Now that kernel calls have finished, copy the output
        ...
        gpuErrchk(cudaMemcpy(output_data, dev_out_data,
                padded_length * sizeof(cufftComplex), cudaMemcpyDeviceToHost));
